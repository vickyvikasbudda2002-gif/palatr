...
export default function FeedPage() {
  ...
  // If auth resolved and no user — redirect (middleware should have caught this)
  if (!user) {
    ...
    <div className="min-h-screen pb-36" style={{ background: "var(--bg)" }}>
      <div className="px-[7%] pt-10">
        ...
        <div className="flex justify-between items-end gap-8 mb-12 flex-wrap">
          <h1 className="font-black leading-tight" style={{ fontSize: "clamp(36px, 5vw, 64px)", letterSpacing: "-2px" }}>
          ...
          <div className="flex gap-3 flex-wrap items-center">
            <input
              ...
              style={{ background: "var(--surface)", border: "1px solid var(--border)", minWidth: "200px", fontFamily: "inherit" }}
            ...
            <select value={sort} onChange={(e) => { const v = e.target.value as SortOption; setSort(v); if (v === "distance") requestLocation(); }}
              className="px-5 py-4 rounded-full text-sm text-white outline-none"
              style={{ background: "var(--surface)", border: "1px solid var(--border)", fontFamily: "inherit", appearance: "none" }}>
      ...
      <ReviewModal isOpen={activeModal === "review"} onClose={() => setActiveModal(null)}
        targetId={selectedRestaurant?.id ?? ""} targetType="restaurant" targetName={selectedRestaurant?.name ?? ""}
        onSubmit={async (data) => { await submitReview(data, "restaurant"); }} />
      <WatchReviewsModal isOpen={activeModal === "watchReviews"} onClose={() => setActiveModal(null)}
        targetId={selectedRestaurant?.id ?? ""} targetType="restaurant" targetName={selectedRestaurant?.name ?? ""} />
      ...
      <NewToCityModal isOpen={activeModal === "newToCity"} city={user.current_city}
        onYes={() => setActiveModal(null)} onNo={() => setActiveModal("top3")} />
      <Top3Modal isOpen={activeModal === "top3"} onBack={() => setActiveModal("newToCity")}
        onClose={() => setActiveModal(null)} restaurants={restaurants}
        homeState={user.home_state} city={user.current_city} />
